...
@author: alankar.
Usage: time python profiles.py van T cr T
"""
import numpy as np
...
if create:
    for select in range(len(tcool_mix_B_tcc)):
        quantity = np.zeros( (till+1, contrasts.shape[0], 7), dtype=np.float64)
        # quantities are (distance, T_wind, rho_wind, v_wind, T_cl, rho_cl, v_cl) for every file at all distances down the wind
        ...
        for file_no in range(0, till+1, analyze_freq):
            ...
            if not(vanilla):
                ...
                quantity[file_no, :, 1] = np.array( [Twind[indx] for indx in temperature_checkpoints_indx] ) # K
                quantity[file_no, :, 2] = np.array( [rho_wind[indx] for indx in temperature_checkpoints_indx] ) # cgs
                ...
                quantity[file_no, :, 1] = np.ones_like(contrasts)*np.average(Twind) # K
                quantity[file_no, :, 2] = np.ones_like(contrasts)*UNIT_DENSITY # cgs
            ...
            for indx, shell_dist in enumerate(quantity[file_no, :, 0]):
                ...
                if indx > 0:
                    if (quantity[file_no, :, 0][indx-1] == shell_dist):
                        for fill in range(3, quantity.shape[-1]):
                            quantity[file_no, indx, fill] = quantity[file_no, indx-1, fill]
                        continue
                if not(vanilla):
                    quantity[file_no, indx, 3] =  CC85wind(shell_dist/distance_ini * diniBdinj)/CC85wind(diniBdinj) # code units
                else:
                    quantity[file_no, :, 3] = np.ones_like(contrasts) # code units
                ...
                if np.sum(cells_chosen) == 0:
                    # quantity[file_no, indx, 1] =  0. # resets Twind; not desired
                    continue
                ...
                quantity[file_no, indx, 4] =  temperature_cl # K
                quantity[file_no, indx, 5] =  density_cl # cgs
                quantity[file_no, indx, 6] =  velocity_cl # code units
            data.close()
        ...
        np.save(f"profiles{'-vanl' if vanilla else ''}_{tcool_mix_B_tcc[select]}.npy", quantity)
        print(f"{directory}")
    with open(f'profiles{"-vanl" if vanilla else ""}.pickle', 'wb') as handle:
...
plot_freq = 5
for select in range( 2 ): # min(len(profiles.keys()),len(tcool_mix_B_tcc))
    ...
    print(f"Reading profiles{'-vanl' if vanilla else ''}_{tcool_mix_B_tcc[select]}.npy: {series.shape}")
    # print(flux.shape)
    # if select != 1:
    #     continue
    for time_indx in range(0, min(till+1, series.shape[0])-59, plot_freq):
        # if time_indx == 20:
        ...
        if time_indx == 0:
            ax1.semilogy(series[time_indx, :, 0], series[time_indx, :, 2]/(mu*mp), linestyle=":", color="k") # color=lines[-1].get_color()) # wind
        if time_indx == 0:
    ...
    for time_indx in range(0, min(till+1, series.shape[0])-59, plot_freq):
        ...
        if time_indx == 0:
            if vanilla:
                ax2.plot(series[time_indx, :, 0], np.ones_like(series[time_indx, :, 0]), linestyle=":", color="k") # wind
            else:
                ax2.plot(series[time_indx, :, 0], series[time_indx, :, 3], linestyle=":", color="k") # wind
        if time_indx == 0:
            xold = series[time_indx, :, 0]
            xnew = xold*rini_B_rcl[select] # units of R_cl
