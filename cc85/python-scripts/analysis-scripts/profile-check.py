...
data_dir = "../../threshold/output-tcoolmBytcc_0.10"
...
velocity = np.array(data[f"/Timestep_{file_no}/vars/vr"]).flatten()*UNIT_VELOCITY/((Edot**(1/2.))*((Mdot*(MSun/yr))**(-1/2.)))
...
plt.hist2d(np.log10(distance), np.log10(pressure), weights=cell_vol, bins=(100,100), density=True, norm=colors.LogNorm())
