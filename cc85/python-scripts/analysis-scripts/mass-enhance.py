# -*- coding: utf-8 -*-
"""
Created on Sun Jan  07 10:38:48 2024
...
cloud_mass_at = []
distance_check = 700 # pc
for i in range(len(tcool_mix_B_tcc)):
    print(f"{root}-c{chi:d},m{mach:.3f},T4e4,t{tcool_mix_B_tcc[i]:.2f},r{rini_B_rcl[i]:.3f}/{filename}")
    distance_check_B_rini = distance_check/Rini
    cloud_mass_at.append( max(0, interp1d(distance_B_rini[i], cloud_mass[i], fill_value="extrapolate")(distance_check_B_rini)) )
...
'''
plt.vlines([Rgo/6,], ymin=1.0, ymax=6.0, colors='k', linestyles='--')
...
plt.yscale("log")
plt.xlabel(r"Cloud size (pc)")
plt.ylabel(r"Cloud mass $M_{\rm cl, max}/M_{\rm cl,ini}$")
# plt.title(r"Cloud $\equiv \ [T<3.3 T_{\rm cl} \ \mathcal{and}\ \rho (d)>10 \rho _{\rm wind}(d)]$")
print("Saving.. ")
plt.savefig(f"cloud-mass_enh-m{mach:.3f}.png", transparent=True, bbox_inches='tight')
plt.close()