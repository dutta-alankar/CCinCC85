...
cc85_profile_vel = velnorm(radius_by_Rinj)*np.sqrt(Edot/(Mdot*MSun/yr))

plt.figure(figsize=(13,10))
plt.plot(scaling_data[:,1], np.gradient(scaling_data[:,1],scaling_data[:,0])*(UNIT_VELOCITY/(km/s)), label="scaling setup")
plt.plot(dummy_cloud_data[:,1], np.gradient(dummy_cloud_data[:,1],dummy_cloud_data[:,0])*(UNIT_VELOCITY/(km/s)), label="spherical setup")
plt.plot(radius_by_Rcl, cc85_profile_vel/(km/s), 'o', label="CC85 solution")
plt.plot(radius_by_Rcl, velnorm(RinibyRinj)*np.sqrt(Edot/(Mdot*MSun/yr))*np.ones_like(radius_by_Rcl)/(km/s), 'o', label="asymptotic solution")
plt.hlines(y=np.sqrt(2*Edot/(Mdot*MSun/yr))/(km/s), xmin=RinibyRcl-15, xmax=700, color="black", linestyle=":")
