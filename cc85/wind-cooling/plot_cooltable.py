...
with plt.style.context('dark_background'):
    ...
    plt.loglog(cooltable[:,0], cooltable[:,1], color="tab:cyan", linewidth=4)
    plt.xlim(2e3,3e8)
    plt.ylim(2e-25, 9e-22)
    plt.xlabel(r"Temperature [$K$]")
    plt.ylabel(r"$\Lambda = \dot{e}/n^2$ [$erg cm^{3} s^{-1}$]")
    plt.savefig("cooltable.png", transparent=True)
