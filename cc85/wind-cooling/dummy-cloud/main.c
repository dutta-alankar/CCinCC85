...
/* =====================================================================
  ...
  /* ----------------------------------------------------
    ...
    #ifdef USE_CATALYST
    do_catalyst_execute(g_stepNumber, g_time, grd, &data);
    #endif
 ...
 ********************************************************************** */
  ...
  double xledg = g_domEnd[0], xledg_all = 0.;
  ...
  DOM_LOOP(k,j,i){
    ...
    if ( ((d->Vc[PRS][k][j][i]/d->Vc[RHO][k][j][i])*pow(UNIT_VELOCITY,2)*(CONST_mp*mu)/CONST_kB) <= factor*Tcl ){
      xc  += d->Vc[RHO][k][j][i]*r[i]*dV;
      mass_trc += d->Vc[RHO][k][j][i]*dV;
    }
    #endif
    xledg = ( (d->Vc[TRC][k][j][i] > 0.2) && r[i]<xledg )? r[i] : xledg;
  ...
  MPI_Allreduce (&xledg, &xledg_all, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
  #else
  xc_all    = xc;
  ...
  g_dist_lab = xc_all;  //wind centric position of the cloud
  g_trctrack = fabs(xledg_all-g_domBeg[0]);
