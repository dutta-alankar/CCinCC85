 ...
 *********************************************************************** */
{
  ...
  if (prank == 0){
    ...
    double tmp, cell_temperature;
    DOM_LOOP(k, j, i) {
      tmp = d->Vc[RHO][k][j][i];
      cell_temperature = (d->Vc[PRS][k][j][i]/d->Vc[RHO][k][j][i])*pow(UNIT_VELOCITY,2)*(CONST_mp*mu)/CONST_kB;
    }
    fprintf (fp, "%12.6e\t\t%12.6e\t\t%12.6e\n", g_dt, tmp, cell_temperature);
    fclose(fp);
