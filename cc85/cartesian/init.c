 ...
 *********************************************************************** */
{
  ...
  TOT_LOOP(k,j,i){
    ...
    double sin_th   = sqrt(x[i]*x[i] + y[j]*y[j])/distance;
    double cos_th   = z[k]/distance;
    double tan_ph   = y[j]/x[i];
    double cos_ph   = x[i]/sqrt(x[i]*x[i] + y[j]*y[j]);
    double sin_ph   = y[j]/sqrt(x[i]*x[i] + y[j]*y[j]);

    double R = sqrt( (x[i]-xIni)*(x[i]-xIni) + (y[j]-yIni)*(y[j]-yIni) + (z[k]-zIni)*(z[k]-zIni) );
    double rByrInj  = (distance/rIni)*rIniByrInj;
    // printLog("Debug: x=%e y=%e z=%e rByrInj=%e\n", x[i], y[j], z[k], rByrInj);
    ...
    d->Vc[VX1][k][j][i]   = vr*x[i]/distance; // sin_th*cos_ph;
    d->Vc[VX2][k][j][i]   = vr*y[j]/distance; // sin_th*sin_ph;
    d->Vc[VX3][k][j][i]   = vr*z[k]/distance; // cos_th;
    d->Vc[TRC][k][j][i]   = 0.;
    if (R <= 1.0) {
      #if WIND_TEST == NO
 ...
 *********************************************************************** */
{
  ...
  if (side == X1_BEG || side == X1_END || side == X2_BEG || side == X2_END || side == X3_BEG || side == X3_END) { /* -- select the boundary side -- */
    BOX_LOOP(box,k,j,i){ /* -- Loop over boundary zones -- */
      double x_val = x[i] + (g_dist_lab-g_dist_start_boost);
      double distance = sqrt(x_val*x_val + y[j]*y[j] + z[k]*z[k]);
      double tan_th   = sqrt(x_val*x_val + y[j]*y[j])/z[k];
      double sin_th   = sqrt(x_val*x_val + y[j]*y[j])/distance;
      double cos_th   = z[k]/distance;
      ...
      d->Vc[VX2][k][j][i]   = vr*y[j]/distance; // vr*sin_th*sin_ph;
      d->Vc[VX3][k][j][i]   = vr*z[k]/distance; // vr*cos_th;
      d->Vc[TRC][k][j][i]   = 0.;
    }
  }
