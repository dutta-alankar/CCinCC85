...
/* ----------------------------------------------
  ...
  if (d!= NULL) {
    DOM_LOOP(kk,jj,ii) {
      dV = grid->dV[kk][jj][ii]; // Cell volume
      dvol += dV;
      trc += d->Vc[RHO][kk][jj][ii]*d->Vc[TRC][kk][jj][ii]*dV;
    }
    #ifdef PARALLEL
    MPI_Allreduce (&trc, &tmp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    trc = tmp;
    MPI_Allreduce (&dvol, &tmp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    dvol = tmp;
    #endif
    printLog("In set grid Before: step %d; t=%f; M_trc = %f; dvol=%f\n", g_stepNumber, g_time, trc, dvol);
  }
  */
  SetGeometry(grid);
  /*
  if (d!= NULL) {
    ...
    dvol = 0.;
    DOM_LOOP(kk,jj,ii) {
      dV = grid->dV[kk][jj][ii-1]; // Cell volume
      dvol += dV;
      trc += d->Vc[RHO][kk][jj][ii]*d->Vc[TRC][kk][jj][ii]*dV;
    }
    #ifdef PARALLEL
    ...
    dvol = tmp;
    #endif
    printLog("In set grid After: step %d; t=%f; M_trc = %f; dvol=%f\n", g_stepNumber, g_time, trc, dvol);
  }
  */
/* ----------------------------------------------
