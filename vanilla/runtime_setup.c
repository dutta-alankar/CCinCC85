...
/* ------------------------------------------------------------
  ...
  #if TRACKING != NO
  double tot_shift_val;
  if(cmd_line->h5restart == YES){
    ...
    fbin = fopen (fout, "r");
    if (fbin == NULL){
      print ("! RestartFromFile(): cannot find dbl.h5.out\n");
      QUIT_PLUTO(1);
    }
    int nlines = 0;
    ...
    fp = fopen(fname, "r"); 
    dummy = fscanf(fp, "%lf", &tot_shift_val);
    fclose(fp);
    fclose(fbin);
    g_tot_shift_val = tot_shift_val;
    //print("Restart shift is %lf\n", tot_shift_val);
    //#ifdef PARALLEL
...
*/
      ...
      #if TRACKING != NO
      if (idim == 0 && cmd_line->h5restart == YES) runtime->patch_left_node[idim][ip] += tot_shift_val;
      #endif
    }
