 ...
 *********************************************************************** */
{
  ...
  /* -- Compute density, velocity and scalars -- */
    ...
    kin = 0.5* v[RHO] * (v[VX1]*v[VX1] + v[VX2]*v[VX2] + v[VX3]*v[VX3]); //m2/u[RHO];

  /* -- Check energy positivity -- */

    #if HAVE_ENERGY
    if (u[ENG] < g_smallPressure/gmm1) {
      #if VERBOSE != NO
      WARNING(
        printLog ("! ConsToPrim: E (%8.2e) < smallEnergy (%8.2e), ", u[ENG], g_smallPressure/gmm1);
        Where (i, NULL);
      )
      #endif
  ...
  /* -- Compute pressure from total energy or entropy -- */
    ...
    if (use_energy){
      ...
      if (v[PRS] < g_smallPressure){
        #if VERBOSE != NO
        WARNING(
          printLog ("! ConsToPrim(): p(E) (%8.2e) < smallPressure (%8.2e), ", v[PRS], g_smallPressure);
          Where (i, NULL);
        )
        #endif
        v[PRS]   = g_smallPressure;
