...
/*!
  ...
  Nov 23, 2021: xmf file generation bug fix by Alankar Dutta (alankardutta@iisc.ac.in)\n
  \date   Jul 21, 2018
*/
/* ///////////////////////////////////////////////////////////////////// */
#include "pluto.h"
 ...
 *********************************************************************** */
{
  ...
  char filename[512], filenamexmf[512], tstepname[32], relhdfloc[512];
  ...
  if (prank == 0) {
    ...
    for (nv = 0; nv < output->nvar; nv++) { /* Write cell-centered variables */
      ...
      fprintf(fxmf, "       <DataItem Dimensions=\"%d %d %d\" NumberType=\"Float\" Precision=\"%d\" Format=\"HDF\">\n",
              ...
              grid->np_int_glob[IDIR], nprec);
      #endif
      fprintf(fxmf, "        %s:%s/vars/%s\n",relhdfloc,tstepname,output->var_name[nv]);
      fprintf(fxmf, "       </DataItem>\n");
